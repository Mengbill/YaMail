            ...
            if(args[0].equalsIgnoreCase("send")){
                if(!(sender instanceof Player)){
                    ...
                    if(args.length == 2){
                        ...
                        if(target == null){
                            player.sendMessage(instance.getConfig().getString("prefix")+"> §4§l该玩家离线或不存在 §a§l已转为离线邮件发送");


                        }else{
                            player.sendMessage(instance.getConfig().getString("prefix")+"> §a§l该玩家当前在线 正在为您发送邮件");
